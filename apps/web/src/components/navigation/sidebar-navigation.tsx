...
function navGroupId(label: string) {
...
export function SidebarNavigation() {
  ...
  return (
    <>
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-80 border-r border-white/10 bg-slate-950/85 px-4 py-5 shadow-2xl shadow-black/30 backdrop-blur-2xl lg:flex lg:flex-col">
        <div className="premium-header-surface rounded-3xl p-4 text-white shadow-2xl shadow-cyan-950/10">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-xs uppercase tracking-[0.24em] text-cyan-200/80">Alpha command shell</p>
              <h1 className="truncate text-lg font-semibold tracking-tight">Insta Intelligence</h1>
            </div>
          </div>
          <div className="mt-4 rounded-2xl border border-white/10 bg-white/10 px-3 py-3 text-xs text-slate-200 shadow-inner shadow-black/10">
            <div className="grid grid-cols-[1fr_auto] items-center gap-3">
              <span className="min-w-0 truncate font-semibold text-slate-100">ACME Agency · Static Alpha workspace</span>
              <span className="h-2 w-2 rounded-full bg-cyan-300 shadow-[0_0_18px_rgba(103,232,249,0.65)]" aria-label="Static Alpha workspace" />
            </div>
            <p className="mt-1 leading-5 text-slate-400">Workspace switching is a preview placeholder; no workspace changes are saved.</p>
            <div className="mt-3 flex flex-wrap gap-2">
              <span className="rounded-full border border-cyan-200/20 bg-cyan-300/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-cyan-100">Preview-only</span>
              <span className="rounded-full border border-emerald-200/20 bg-emerald-300/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-emerald-100">No live sync</span>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 space-y-5 overflow-y-auto pr-1" aria-label="Private app product navigation">
          {navigationGroups.map((group) => {
            const groupId = navGroupId(group.label);

            return (
              <section key={group.label} aria-labelledby={groupId} className="rounded-3xl border border-white/[0.06] bg-white/[0.025] p-2.5">
                <div className="px-1.5">
                  <h2 id={groupId} className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400">
                    {group.label}
                  </h2>
                  <p className="mt-1 text-[11px] leading-4 text-slate-600">{group.summary}</p>
                </div>
        ...
        <div className="mt-5 rounded-3xl border border-emerald-300/20 bg-emerald-300/10 p-4 text-xs text-slate-300 shadow-inner shadow-emerald-950/20">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <Badge variant="outline" className="rounded-full border-emerald-300/25 bg-emerald-300/20 text-emerald-100">Official-source first</Badge>
          </div>
          <p className="mt-2 leading-5">Public, owned, consented, or approved-source data only. No live Instagram data is collected in Alpha.</p>
          <p className="mt-2 rounded-2xl border border-white/10 bg-slate-950/30 p-3 leading-5 text-slate-400">No scraping, private account access, hidden surveillance, or anti-bot bypass. Provider approval may be required.</p>
        </div>
      </aside>
          ...
          {mobileItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={active ? "mt-0.5 block text-[10px] text-cyan-100/70" : "mt-0.5 block text-[10px] text-slate-500"}>Static preview route</span>
              </Link>
            );
          })}
        </div>
